    ...
    computed: {
        generateMenus() {
            return [
                ...
                // if you use link (:to) for external url, it will be like http://localhost:8080/https://twitter.com/Sara_Hoshikawa
                // don't worry the vue itself will use the defined one without giving an error
                { icon: 'mdi-surround-sound', text: this.$root.$t('voice-button'), link: '/', color: 'yellow darken-1' },
                { 
                    icon: 'mdi-youtube', 
                ...
                { icon: 'mdi-information', text: this.$root.$t('about'), link: '/about', color: 'white' },
            ]
        }
    }
}
